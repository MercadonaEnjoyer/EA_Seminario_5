...
export class PostController {
    ...
    public async getUserFromPost(req: Request, res: Response){
        try{
            if (req.params.id) {
                const post_filter = { _id: req.params.id };
                // Fetch user
                const post_data = await this.post_service.populatePost(post_filter);

                // Send success response
                return res.status(200).json({ data: post_data.author, message: 'Creator of post'});
            } else {
                return res.status(400).json({ error: 'Missing fields' });
            }
        }catch(error){
